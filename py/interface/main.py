...
from SnakeGameController import Controller, GameState, IpcCommands, FoodType, Direction # noqa: E402
...
AVAILABLE_MAP_SIZES = [
    (10, 10), (20, 20), (30, 30), (40, 40), (20, 10), (10, 20)
]
...
def update_layout(map_width, map_height):
    """
    Oblicza rozmiar komórki i offsety tak, aby mapa zajmowała 80% wysokości ekranu.
    Następnie przeładowuje grafiki w nowym rozmiarze.
    """
...
def restart_game_process(width, height):
    ...
    if game_process:
        ...
        except: game_process.kill()

    try:
        print(f"[SYSTEM] Restarting C++ game with size {width}x{height}...")
        game_process = subprocess.Popen(
            [str(cpp_game_path), str(width), str(height)], 
...
def draw_board(screen, width, height):
    ...
    for y in range(height):
        for x in range(width):
            ...
            is_even = (x + y) % 2 == 0
            if is_even:
                if b1: screen.blit(b1, (pos_x, pos_y))
                else: pygame.draw.rect(screen, (30, 30, 30), (pos_x, pos_y, CELL_SIZE, CELL_SIZE))
            else:
                if b2: screen.blit(b2, (pos_x, pos_y))
                else: pygame.draw.rect(screen, (40, 40, 40), (pos_x, pos_y, CELL_SIZE, CELL_SIZE))
...
def draw_snake(screen, snake_head, snake_body, snake_dir):
    ...
    if img_body and not img_corner: img_corner = img_body

    # --- KROK 1: RYSOWANIE CIAŁA I OGONA ---
    for i in range(1, len(segments)):
        segment = segments[i]
        if segment == tail_pos and i != len(segments) - 1: continue
        ...
        if next_seg is None:
            ...
            if prev_seg[0] == segment[0] == next_seg[0] or \
               prev_seg[1] == segment[1] == next_seg[1]:
                angle = 0
                if prev_seg[1] == segment[1]: angle = 90 
                ...
                angle = 0
                if corner_sum == (1, 1): angle = 0      
                elif corner_sum == (-1, 1): angle = -90 
                elif corner_sum == (-1, -1): angle = 180 
                elif corner_sum == (1, -1): angle = 90  
    ...
    if img_head:
        angle = 0
        if snake_dir == Direction.DOWN: angle = 180
        elif snake_dir == Direction.LEFT: angle = 90
        elif snake_dir == Direction.RIGHT: angle = -90
...
def main():
    ...
    while running:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if data and data.game_state == GameState.PLAYING and not aiMode and not algoMode:
                    cmd = None
                    if event.key in [pygame.K_w, pygame.K_UP]: cmd = IpcCommands.MOVE_UP
                    elif event.key in [pygame.K_s, pygame.K_DOWN]: cmd = IpcCommands.MOVE_DOWN
                    elif event.key in [pygame.K_a, pygame.K_LEFT]: cmd = IpcCommands.MOVE_LEFT
                    elif event.key in [pygame.K_d, pygame.K_RIGHT]: cmd = IpcCommands.MOVE_RIGHT
                    if cmd: controller.send_command(cmd)

                # --- MENU ---
                if data and data.game_state == GameState.MENU:
                    if menu_sub_state == 0:
                        if event.key in [pygame.K_w, pygame.K_UP]: menu_selection = (menu_selection - 1) % len(menu_options)
                        elif event.key in [pygame.K_s, pygame.K_DOWN]: menu_selection = (menu_selection + 1) % len(menu_options)
                            ...
                            if menu_selection == 0:
                                ...
                                controller.send_command(IpcCommands.START_GAME)
                            elif menu_selection == 3: menu_sub_state = 1
                        ...
                        if available_models:
                             if event.key in [pygame.K_w, pygame.K_UP]: model_selection_idx = (model_selection_idx - 1) % len(available_models)
                             elif event.key in [pygame.K_s, pygame.K_DOWN]: model_selection_idx = (model_selection_idx + 1) % len(available_models)
                             elif event.key in [pygame.K_RETURN, pygame.K_KP_ENTER]:
                                current_model_name = available_models[model_selection_idx]
                                menu_sub_state = 0
                    
                    elif menu_sub_state == 2:
                        if event.key in [pygame.K_w, pygame.K_UP]: map_menu_idx = (map_menu_idx - 1) % len(AVAILABLE_MAP_SIZES)
                        elif event.key in [pygame.K_s, pygame.K_DOWN]: map_menu_idx = (map_menu_idx + 1) % len(AVAILABLE_MAP_SIZES)
                        elif event.key in [pygame.K_RETURN, pygame.K_KP_ENTER]:
        ...
        if new_data is not None: data = new_data
        if data is None:
            time.sleep(0.001)
            continue
        if data_updated: should_render = True
            ...
            if aiMode or algoMode:
                if data.version != last_ai_decision_version:
                    ...
                    if calculated_direction is not None:
                        ...
                        is_opposite = False
                        if (current_dir == 0 and dir_val == 1) or (current_dir == 1 and dir_val == 0) or \
                           (current_dir == 2 and dir_val == 3) or (current_dir == 3 and dir_val == 2):
                            is_opposite = True

                        if not is_opposite and dir_val != current_dir:
                            if dir_val == 0: cmd = IpcCommands.MOVE_UP
                            elif dir_val == 1: cmd = IpcCommands.MOVE_DOWN
                            elif dir_val == 2: cmd = IpcCommands.MOVE_LEFT
                            elif dir_val == 3: cmd = IpcCommands.MOVE_RIGHT
        ...
        if should_render:
            ...
            if data.game_state == GameState.MENU:
                if menu_sub_state == 0:
                    ...
                    draw_legend(screen, font_small, ["[ENTER] Wybierz", "[W / S] Nawigacja", "[Q] Zamknij"])
                     ...
                     for i, m_file in enumerate(available_models):
                        ...
                        color = (200, 200, 200)
                        if is_selected: color = (255, 255, 0)
                        if is_active: color = (0, 255, 0) 
                ...
                if food_img:
                    ...
                    pygame.draw.rect(screen, c, (OFFSET_X + fx * CELL_SIZE, OFFSET_Y + fy * CELL_SIZE, CELL_SIZE, CELL_SIZE))
                ...
                if data.game_state == GameState.GAME_OVER:
                    ...
                    screen.blit(font_medium.render(f"Wynik: {data.score}", True, (255, 255, 255)), (SCREEN_WIDTH // 2 - 50, SCREEN_HEIGHT // 2 + 10))
